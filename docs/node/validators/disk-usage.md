...
speed and transaction amount. For Tenet, we are talking about close to
100GB of disk usage in first two months.
...
I believe this was disabled by default on Tenet, but listing it in any
...
will risk database corruption if the `tenetd` is killed for any reason.
